...
export function PlatformPulse() {
  ...
  return (
    <section className="glass-panel rounded-3xl p-5 sm:p-7" aria-label="Continuous security command center">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Recent scans" value={data?.summary.scan_count ?? "—"} detail="Persisted work queue" accent="text-emerald-200" /><Metric label="Targets observed" value={data?.summary.target_count ?? "—"} detail="Distinct target origins" accent="text-cyan-200" /><Metric label="Active investigations" value={data?.summary.active_scan_count ?? "—"} detail="Queued or in progress" accent="text-amber-200" /><Metric label="Completed reports" value={data?.summary.state_counts.COMPLETED ?? "—"} detail="Ready for review" accent="text-emerald-200" /></div>
      <div className="glass-panel-subtle mt-5 overflow-hidden rounded-2xl">
