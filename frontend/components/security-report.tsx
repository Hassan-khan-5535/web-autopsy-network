...
export function SecurityReport({ report }: { report: SecurityReportResponse }) {
  ...
  const filteredFindings = useMemo(() => report.technical_findings.filter((finding) => (severity === "all" || finding.severity === severity) && finding.confidence >= Number(minimumConfidence)), [report.technical_findings, severity, minimumConfidence]);
  return (
    <section id="security-posture-report" className="space-y-6 rounded-2xl border border-indigo-500/25 bg-[#0b1714] p-6">
      ...
      <div className="rounded-xl border border-indigo-500/15 bg-black/20 p-5">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><h3 className="text-sm font-semibold text-indigo-100">Technical Findings</h3><p className="mt-1 text-xs text-emerald-100/50">Severity, confidence, affected location, evidence state, deterministic risk, remediation, and mapped references.</p></div><div className="flex flex-wrap items-end gap-2"><label className="text-[10px] font-mono uppercase tracking-wider text-emerald-100/45">Severity<select value={severity} onChange={(event) => setSeverity(event.target.value)} className="mt-1 block rounded border border-indigo-400/25 bg-[#08110f] px-2 py-1.5 text-xs text-indigo-100"><option value="all">All</option><option value="critical">Critical</option><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option><option value="info">Info</option></select></label><label className="text-[10px] font-mono uppercase tracking-wider text-emerald-100/45">Min. confidence<select value={minimumConfidence} onChange={(event) => setMinimumConfidence(event.target.value)} className="mt-1 block rounded border border-indigo-400/25 bg-[#08110f] px-2 py-1.5 text-xs text-indigo-100"><option value="0">Any</option><option value="50">50%+</option><option value="70">70%+</option><option value="90">90%+</option></select></label><span className="pb-1 text-xs font-mono text-indigo-200">{filteredFindings.length}/{report.technical_findings.length}</span></div></div>
        <div className="mt-4 space-y-3">{filteredFindings.length ? filteredFindings.map((finding) => <details key={finding.id} className="rounded-lg border border-indigo-500/15 bg-[#08110f] p-4"><summary className="cursor-pointer list-none"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-indigo-100">{finding.statement}</p><p className="mt-1 max-w-3xl truncate text-xs font-mono text-emerald-100/50" title={finding.subject}>{finding.rule_id} · {finding.subject}</p></div><span className={`rounded-full border px-2.5 py-1 text-xs font-mono ${severityStyle[finding.severity] ?? severityStyle.info}`}>{finding.severity} · {finding.confidence}%</span></div></summary><div className="mt-4 grid gap-4 border-t border-indigo-500/10 pt-4 text-xs leading-5 text-emerald-100/65 md:grid-cols-2"><div><p className="font-mono uppercase tracking-wider text-emerald-100/40">Affected location</p><p className="mt-1 break-all">{finding.affected_url ?? finding.subject}{finding.affected_parameter ? ` · parameter: ${finding.affected_parameter}` : ""}</p><p className="mt-3 font-mono uppercase tracking-wider text-emerald-100/40">Evidence</p><p className="mt-1">State: {finding.evidence_state} · quality: {finding.evidence_quality} · classification: {finding.classification}</p><pre className="mt-2 max-h-28 overflow-auto whitespace-pre-wrap rounded bg-black/30 p-2 text-[10px] text-emerald-100/50">{JSON.stringify(finding.evidence, null, 2)}</pre></div><div><p className="font-mono uppercase tracking-wider text-emerald-100/40">Remediation</p><p className="mt-1">{finding.remediation}</p><p className="mt-3 font-mono uppercase tracking-wider text-emerald-100/40">References</p><p className="mt-1 flex flex-wrap gap-2">{finding.references.length ? finding.references.map((reference) => <a key={reference.label} href={reference.url} target="_blank" rel="noreferrer" className="text-indigo-300 underline decoration-indigo-400/40 underline-offset-2">{reference.label}</a>) : "No reference mapping available"}</p><p className="mt-3 text-emerald-100/45">Risk: {finding.risk_score} ({finding.risk_band}) · {finding.limitations || "No additional limitation recorded."}</p></div></div></details>) : <p className="rounded-lg border border-indigo-500/15 bg-[#08110f] p-4 text-sm text-emerald-100/55">No persisted findings match the current severity and confidence filters.</p>}</div>
      </div>
      {report.executive_summary.limitations.length > 0 && <div className="rounded-lg border border-amber-300/15 bg-amber-500/[0.04] p-3 text-xs text-amber-100/60"><p className="font-mono uppercase tracking-wider text-amber-300/70">Coverage notes</p><p className="mt-1">{report.executive_summary.limitations.join(" ")}</p></div>}
    </section>
  );
}
